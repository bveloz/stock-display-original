...
Recognizer_Marubozu::Recognizer_Marubozu() {
	this->patternName = "Marubozu";
	//set the name of the recognizer
	this->patternLength = 1;
	//set the length of the recognizer
};
bool Recognizer_Marubozu::recognize(List<SmartCandlestick^>^ smartCsList, int index) {
	bool value;
	...
	//check that the index is out of bounds do nothing
		...
		//otherwise
		if (smartCsList[index]->bodyRange > smartCsList[index]->range * .8)
		{
