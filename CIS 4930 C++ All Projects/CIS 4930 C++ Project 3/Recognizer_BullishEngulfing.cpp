...
Recognizer_BullishEngulfing::Recognizer_BullishEngulfing() {
	this->patternName = "Bullish Engulfing";
	//set the name of the recognizer
	this->patternLength = 2;
	//set the length of the recognizer
};
bool Recognizer_BullishEngulfing::recognize(List<SmartCandlestick^>^ smartCsList, int index) {
	bool value;
	...
	//check that the index is out of bounds do nothing
		...
		//otherwise
		if (smartCsList[index]->open < smartCsList[index + 1]->close && smartCsList[index]->close > smartCsList[index + 1]->open &&
		...
		{
			//if this candlestick open is less than right candlestick close and this candlestick close is greater than right candlestick open
