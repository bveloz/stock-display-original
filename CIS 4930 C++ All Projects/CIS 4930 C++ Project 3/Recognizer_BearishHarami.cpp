...
Recognizer_BearishHarami::Recognizer_BearishHarami() {
	this->patternName = "Bearish Harami";
	//set the name of the recognizer
	this->patternLength = 2;
	//set the length of the recognizer
};
bool Recognizer_BearishHarami::recognize(List<SmartCandlestick^>^ smartCsList, int index) {
	bool value;
	...
	//check that the index is out of bounds do nothing
			...
			smartCsList[index]->high > smartCsList[index + 1]->high && smartCsList[index]->low < smartCsList[index + 1]->low)
		{
			...
			//and if this candlestick bullish and right candlestick is bearish	
